...
nonisolated struct ProjectManifest: Codable, Sendable {
    ...
    /// message all read this, so the three cannot drift apart — which they had: the version was
    /// bumped to 9 and only the manifest check was widened, leaving the header check at 1...8, so
    /// every saved document failed to reopen with `.version(9)`. Widen this in the same change that
    /// bumps `current`.
